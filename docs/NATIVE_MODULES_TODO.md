...
- [x] 实现：
  ...
  - [x] `lua_is*` 系列常用入口：
    ...
    - [x] `lua_iscfunction`
      - [x] 当前对宿主侧 Go closure 和 native C closure wrapper 返回 true；Lua closure、带 `__call` 的 table、非 function 和 none 返回 false。
    - [x] `lua_isuserdata`
      - [x] 当前对 full userdata 和 lightuserdata 返回 true，对 nil、number、string、table、none 返回 false。
  ...
  - [x] `lpeg` 或等价纯 C 模块验收：覆盖 userdata、metatable、registry 和复杂 C function 行为。
    ...
    - [x] 完整 `third_party/lpeg/test.lua`、复杂 capture、grammar 和错误边界已通过 macOS arm64 native 验收。
      ...
      - [x] 当前完整测试推进到 1159 行；同一 pattern 独立运行返回 18，但完整前序状态后返回 12，疑似 LPeg match-time capture / named capture 前序状态污染，需继续缩小最小复现。
        ...
        - [x] 2026-07-07 门禁：使用 `gopls check internal/native/capi_userdata_native.go internal/native/capi_userdata_native_test.go internal/native/capi_stack_native.go internal/native/state_handle_native.go` 复核后，新增 `TestNativeLuaGetUserValueRespectsCurrentCFrameBase`，验证当前 C frame 为空时 `lua_getuservalue(L, 1)` 不会穿透读取外层 full userdata 的 user value，只在当前帧压入失败安全 nil。该门禁约束 native userdata/user value 正索引可见性，仍不把 LPeg select-count 退化归因到该组合。
        - [x] 2026-07-07 门禁：使用 `gopls check internal/native/capi_metatable_native.go internal/native/capi_metatable_native_test.go internal/native/capi_stack_native.go internal/native/state_handle_native.go` 复核后，新增 `TestNativeLuaGetMetatableRespectsCurrentCFrameBase`，验证当前 C frame 为空时 `lua_getmetatable(L, 1)` 不会穿透读取外层 table 的 raw metatable，也不会向全局栈压入隐藏对象。该门禁约束 native metatable 正索引可见性，仍不把 LPeg select-count 退化归因到该组合。
        - [x] 2026-07-07 修复：使用 `gopls check runtime/gc.go runtime/gc_test.go runtime/value.go runtime/state.go` 复核后，补齐 GC root/weak sweep 对 `GoClosureWithUpvalues` 显式 upvalue 的建模，并让 weak sweep 从主栈或当前 call frame 中的显式 Go closure 继续递归扫描其 upvalue；同时保留 suspended coroutine cycle finalizer 延迟语义，不把协程入口无条件当普通强根。新增模块无关测试 `TestGCRootsGoClosureWithUpvaluesRoot` 与 `TestSweepWeakTablesKeepsGoClosureUpvalueValues`，锁定 native C closure、`string.gmatch` 与 `dofile` trampoline 这类显式 Go closure upvalue 不会被 root 采样或 weak sweep 漏扫。复跑 `PROBE_CONTINUE_ON_CRASH=1 PROBE_MODES="select-count-global-value-error-number select-count-global-value-type-number" ./scripts/probe-native-lpeg-select-count.sh` 后，`error-number` 仍为 `result=12 class=bad`、`type-number` 仍为 `result=18 class=good`，因此该 GC/root 缺口需要修复，但不是当前 LPeg select-count 退化的直接充分根因。
        ...
        - [x] 2026-07-07 复核：继续为 `PROBE_SELECTED_WARMUP_KIND` 增加纯 Lua 控制组 `string-close/string-open/string-eq/string-a/string2-close/string-concat-close/table-key-close`。短矩阵显示纯字符串 `']'` 前置或后置、`']' .. ''`、以及 `{ [']'] = true }` 均恢复为 `18 good`；但 `'['`、`'='`、`'a'` 均保持 `error-number=12 bad/type-number=18 good`，`']]'` 则让两组都退化为 `12 bad`。因此恢复不必经过 LPeg `m.P']'` 或 native TChar 构造，当前根因窗口前移到 Go VM/string 短字符串值、intern/cache、寄存器残留与后续 LPeg 构造读取之间的通用交互；后续应优先比较单字符 `]` 的字符串值生命周期、常量表加载、字符串 interning、GC root 和固定返回 CALL 临时槽，而不是继续按 LPeg pattern API 做特向性分析。
        - [x] 2026-07-07 复核：继续为纯 Lua `]` 控制组增加 `string-close-dead-branch/string-close-dead-function/string-close-called-function`。短矩阵显示主 chunk 中 `if false then probe_warmup = ']' end` 未执行分支仍恢复为 `18 good`，但嵌套函数内未调用的 `return ']'` 以及实际调用该嵌套函数返回 `']'` 都不恢复，仍为 `error-number=12 bad/type-number=18 good`。因此恢复条件不只是运行时拿到 `]` 字符串值或字符串 intern，而更像当前主 Proto 常量表/字节码形态中提前出现同字符常量，改变了后续默认 tail 的常量索引、LOADK 形态或固定返回 CALL 后寄存器历史；下一步应优先落到 bytecode/Proto 常量表和 VM CALL 后临时槽清理的通用语义对照。
        - [x] 2026-07-07 复核：扩展 `scripts/probe-native-lpeg-select-bytecode.sh`，将 baseline、直接 `string-close`、未执行 dead branch、未调用 nested function 和已调用 nested function 五组落到主 Proto/子 Proto 反汇编。baseline 中主 Proto `"]"` 常量为 `constants[16]`、`maxstack=10`；直接 `string-close` 与 dead branch 两个恢复组都把主 Proto `"]"` 提前到 `constants[0]`，`maxstack=11`，后续 close-head 构造使用 `K(0)`；nested function 未调用/已调用两个坏组中，主 Proto `"]"` 仍为 `constants[16]`，只有子 Proto 持有 `constants[0]="]"`。因此恢复条件进一步收敛到主 Proto 常量表顺序/索引与默认 tail 构造 bytecode/register shape，而不是运行时字符串对象、字符串 intern 或 nested Proto 字符串常量；下一步应继续用模块无关测试或 trace 对比 `LOADK` 常量索引、固定返回 CALL 临时槽和 LPeg 组合构造读取路径。
        - [x] 2026-07-07 复核：继续为 count/bytecode probe 增加 `string-close-after-dead-pad`、`string-close-dead-branch-after-dead-pad` 和 `string-close-dead-branch-after-dead-pads8`。短矩阵显示即使 `]` 只出现在主 chunk 未执行分支，且前面插入 1 个或 8 个未执行 pad 字符串常量，`error-number` 与 `type-number` 仍均恢复为 `18 good`；对应 bytecode 中主 Proto `"]"` 分别为 `constants[1]` 与 `constants[8]`，而 nested function 对照主 Proto 仍为 `constants[16]` 且 `error-number=12 bad`。因此已证伪“必须是主 Proto 的 `K(0)` 常量索引”是恢复条件；当前更像主 Proto 早期常量/额外未执行 LOADK 字节码改变了默认 tail 构造前的寄存器、PC 或常量装载历史，而不是某个固定常量索引或运行时 warmup 值。
        - [x] 2026-07-07 复核：继续为 count/bytecode probe 增加 `string-dead-pads8-only` 控制组，只插入 8 个未执行 pad 字符串常量，不提前放入 `]`。短矩阵显示该组 `error-number=12 bad`，且原本 good 的 `type-number` 也退化为 `12 bad`；对应 bytecode 中默认 tail 的主 Proto `"]"` 被推到 `constants[24]`。与 `string-close-dead-branch-after-dead-pads8` 的 `constants[8]="]"` 且两组均 good 对照后，可证伪“额外死代码/常量 padding 本身恢复”是根因；恢复仍需要主 Proto 在默认 tail 之前包含同字符 `]` 常量，但不是固定索引 `K(0)`。
        - [x] 2026-07-07 复核：将 `string-close-dead-branch-after-dead-pads` 改为 `PROBE_STRING_CLOSE_PAD_COUNT=<n>` 可配置，并用 0/4/8/9/10/11/12/15/16 做二分式短矩阵。结果显示 pad=0/4/8/9 时 `error-number` 与 `type-number` 均为 `18 good`，pad=10/11/12/15/16 时两组均为 `12 bad`；bytecode 对照确认 pad=9 时主 Proto `"]"` 为 `constants[9]`，pad=10 时为 `constants[10]`。因此当前强信号收敛为主 Proto `"]"` 常量索引存在 `K(9)`/`K(10)` 翻转边界，下一步应优先检查通用 Proto 常量、GC root/mark、常量扫描上限、closure child proto 常量递归和 native 可见根采样是否存在前 10 项边界，而不是继续按 LPeg API 或字符语义扩展矩阵。
        - [x] 2026-07-07 证伪：使用 `gopls check runtime/vm.go runtime/vm_test.go bytecode/proto.go` 复核后，新增默认构建测试 `TestVMLoadKLoadsDoubleDigitConstantIndex`，直接构造 11 个 Proto 常量并执行 `LOADK constants[10]`。`go test ./runtime -run TestVMLoadKLoadsDoubleDigitConstantIndex -v` 已通过，说明 VM 常量读取本身不存在“只支持前 10 项”边界；K(9)/K(10) 信号后续应继续落到常量布局引发的寄存器/对象生命周期、GC/root 或 native 可见栈交互，而不是 LOADK 读取逻辑。
        - [x] 2026-07-07 门禁：使用 `gopls check internal/native/capi_stack_native.go internal/native/capi_stack_native_test.go internal/native/capi_type_native.go internal/native/state_handle_native.go` 复核后，新增 `TestNativeCAPIUpvaluePseudoIndexesUseCurrentCFrame`，验证 `lua_upvalueindex(i)` 从当前 C closure 调用帧 upvalue 快照读取、可被 `lua_pushvalue` 复制、越界 upvalue 表现为 none，且嵌套 C frame 的 upvalue 会遮蔽并在退出后恢复外层 frame。该门禁未改生产代码，证实当前 `lua_upvalueindex` 读路径不是 select-count 退化的直接缺口，后续继续按通用构造期生命周期、常量布局与 native 可见根交互排查。
        - [x] 2026-07-07 修复：使用 `gopls check runtime/gc.go runtime/gc_test.go runtime/vm.go runtime/value.go` 复核后，将 GC/root、弱表强根、ephemeron 扩展和 weak sweep 中的 Lua closure upvalue 扫描统一收敛到当前 `UpvalueCell` 值；只有缺失 cell 时才回退创建时 `Upvalues` 快照。新增 `TestGCRootsLuaClosureUpvalueCellCurrentValue` 与 `TestSweepWeakTablesKeepsLuaClosureUpvalueCellValues`，覆盖外层局部在闭包创建后改写时，GC 不应只保留过期快照。复跑 `PROBE_CONTINUE_ON_CRASH=1 PROBE_MODES="select-count-global-value-error-number select-count-global-value-type-number" ./scripts/probe-native-lpeg-select-count.sh` 后，`error-number` 仍为 `result=12 class=bad`、`type-number` 仍为 `result=18 class=good`，因此该 upvalue cell GC 缺口需要修复，但不是当前 LPeg select-count 退化的直接充分根因。
        - [x] 2026-07-07 复核：为 `scripts/probe-native-lpeg-select-count.sh` 增加 `select-count-local-callee` 与 `select-count-gtable-callee` 固定返回来源，比较直接 `select(...)`、局部别名 `select_fn(...)` 和 `_G["select"](...)` 三种调用形态。在唯一 number padding `pad=4/5` 下，三种 `select` 来源与 `error+number` 组合均为 `result=12 class=bad`，而 `builtin-rawget` 对照仍为 `result=18 class=good`。因此已证伪全局查找形态、局部别名搬运或 `_G` table 读取是充分根因；后续应继续聚焦 `select`/vararg 固定返回 CALL 的返回写回、非结果实参槽生命周期与 native LPeg 构造期可见根组合，而不是按函数名或查找路径特例化。
        - [x] 2026-07-07 门禁：使用 `gopls check internal/native/capi_userdata_native.go internal/native/capi_userdata_native_test.go internal/native/capi_table_native.go internal/native/capi_table_native_test.go internal/native/capi_stack_native.go internal/native/state_handle_native.go` 复核后，新增 `TestNativeLuaUserValueJoinTablesWithShiftedRawSetI`，模拟 LPeg `joinktables/concattable` 这类通用构造期 C API 组合：读取两个 userdata user value table、在 `lua_rawgeti` 压栈后用 `idx2-1` 写入新 table、再 `lua_setuservalue` 挂回新 userdata，并确认临时 ktable 不泄漏到外层栈。该门禁通过，说明 userdata uservalue table 合并、`rawgeti/rawseti` 负索引修正和 `setuservalue` 弹栈不是当前 select-count 退化的直接缺口。
        - [x] 2026-07-07 证伪：使用 `gopls check runtime/vm.go lua/api.go lua/api_test.go stdlib/base/base.go stdlib/base/base_call_trace_test.go` 复核后，新增固定返回 CALL 后非结果实参槽清理边界：普通 Lua closure 与无 native C frame 的 Go 回调可在写回后清理，native C closure/`GoClosureWithUpvalues` 保守跳过，且活动 local 按 CALL 后 PC 保留。`go test ./lua -run 'TestWriteLuaCallResultsSingleReturnFastPath|TestDoStringCallTemporaryCleanup'` 与 `go test -tags call_trace ./stdlib/base -run 'TestBaseLuaCallTrace(FixedResultPairs|CalleeRegisterLifetimes)' -v` 已通过，trace 显示 `select`/`rawequal` 非结果实参槽变为 `nil`，活动 local 保留；但 LPeg select-count probe 仍为 `select-count-global-value-error-number result=12 class=bad`、`fixed-result-global-value-select-count-error-number result=12 class=bad`、`fixed-result-global-value-builtin-rawequal-error-number result=12 class=bad`，`rawget` 对照仍为 `18 good`。另外，本轮未保留的“所有固定 CALL 一律清理”实验会让相关 LPeg probe `class=crash exit=2`，因此已证伪“简单清理固定 CALL 非结果槽”是充分修复；后续应转向 native C closure wrapper/root/GC 生命周期，而不是继续扩大无差别清理范围。
        - [x] 2026-07-07 门禁：使用 `gopls check internal/native/capi_cfunction_native_test.go internal/native/state_handle_native.go runtime/gc.go` 复核后，新增 `TestNativeCFrameVisibleStackRootsWeakSweep`，验证当前 C frame 可见栈上的临时 table 会作为 `State.stack` 根参与 weak table sweep，保活只由该临时栈值间接引用的 weak value。该测试通过，证伪“native C API 临时栈值未进入 weak sweep 强根”是当前 select-count 退化的直接缺口；后续继续聚焦 native C closure wrapper、构造期对象挂接和 registry/metatable/userdata 可见根组合。
        - [x] 2026-07-07 门禁：使用 `gopls check internal/native/capi_cfunction_native.go internal/native/capi_cfunction_native_test.go internal/native/state_handle_native.go internal/native/capi_stack_native.go` 复核后，新增 `TestNativeLuaPushCClosureCapturesMultipleUpvaluesAndKeepsVisiblePrefix`，验证 `lua_pushcclosure(nup=2)` 只消费当前 C frame 栈顶 upvalue 后缀、保留同一 C frame 内非 upvalue 前缀，并按原始顺序保存 table/integer upvalue。`CGO_ENABLED=1 go test -tags native_modules ./internal/native -run TestNativeLuaPushCClosureCapturesMultipleUpvaluesAndKeepsVisiblePrefix -v` 已通过；复跑 `PROBE_CONTINUE_ON_CRASH=1 PROBE_MODES="select-count-global-value-error-number select-count-global-value-type-number" ./scripts/probe-native-lpeg-select-count.sh` 后仍为 `error-number result=12 class=bad`、`type-number result=18 class=good`，因此该门禁证伪“C closure 多 upvalue 构造弹栈越界或对象挂接顺序错误”是当前 select-count 退化的直接充分根因。
        - [x] 2026-07-07 修复：使用 `gopls check runtime/gc.go runtime/gc_test.go` 复核后，补齐 `SnapshotGCRoots` 对主栈、活动寄存器和协程栈中 closure 值的 upvalue 展开，新增 `TestGCRootsStackGoClosureWithUpvaluesRoot` 覆盖栈上 `GoClosureWithUpvalues` 的显式 upvalue 根采样。该修复面向 native C closure/string.gmatch/dofile trampoline 等通用 Go closure upvalue 生命周期，不按 LPeg pattern 特判；复跑 `PROBE_CONTINUE_ON_CRASH=1 PROBE_MODES="select-count-global-value-error-number select-count-global-value-type-number" ./scripts/probe-native-lpeg-select-count.sh` 后仍为 `error-number result=12 class=bad`、`type-number result=18 class=good`，因此该 root snapshot 缺口需要修复，但不是当前 select-count 退化的充分根因。
        - [x] 2026-07-07 修复：使用 `gopls check runtime/gc.go runtime/gc_test.go` 复核后，补齐 userdata user value/raw metatable 关联边在 `hasReachableWeakTables`、自动 finalizer 可达性、suspended thread 引用判断、weak-key association 和 ephemeron 固定点传播中的图遍历分支。新增 `TestFullGCKeepsWeakTableFlagThroughUserdataAssociation`，覆盖弱表只经 native full userdata user value 或 raw metatable 可达时，完整 GC 后 `hasWeakTables` 不被误清。复跑 LPeg select-count 短 probe 后仍为 `error-number result=12 class=bad`、`type-number result=18 class=good`，因此该 userdata/weak sweep 一致性缺口需要修复，但不是当前退化充分根因。
        - [x] 2026-07-07 修复：使用 `gopls check runtime/state.go runtime/gc.go runtime/gc_test.go internal/native/state_handle_native.go internal/native/capi_cfunction_native_test.go` 复核后，新增 runtime external GC root frame 机制，并在 native C function frame 进入/退出时把当前 C closure upvalue 快照同步登记为临时强根。新增 `TestSweepWeakTablesKeepsExternalGCRootValues` 与 `TestNativeCFrameUpvaluesRootWeakSweep`，覆盖 external root 参与 root snapshot/weak sweep、以及 C closure 调用期 upvalue 只通过该帧可达时不得被 weak sweep 清理。复跑短 probe 后仍为 `error-number result=12 class=bad`、`type-number result=18 class=good`，因此该 C closure 调用期 native 可见根缺口需要修复，但不是当前 LPeg select-count 退化的充分根因。
        - [x] 2026-07-07 门禁：使用 `gopls check runtime/gc.go runtime/gc_test.go internal/native/capi_metatable_native.go internal/native/capi_metatable_native_test.go` 复核后，新增 `TestSweepWeakTablesKeepsRegistryAssociationValues`，验证 registry 中保存的命名元表 table，其 raw metatable 间接引用对象可保活 weak value；同时验证 registry 中保存的 userdata，其 user value 与 raw metatable 关联边也会保活 weak value。该默认构建门禁通过，证伪 registry/metatable 关联边漏扫是当前 select-count 退化的直接缺口。
        - [x] 2026-07-07 门禁：使用 `gopls check internal/native/capi_call_native.go internal/native/capi_call_native_test.go internal/native/capi_userdata_native.go internal/native/capi_stack_native.go internal/native/state_handle_native.go` 复核后，新增 `TestNativeLuaCallKFixedResultKeepsConstructedUserdataRoots`，验证固定返回 `lua_callk` 写回后，同一 C frame 继续创建 native full userdata、构造 user value table 并间接引用 weak value 时，weak sweep 不会误清理该对象。短 probe 仍为 `error-number result=12 class=bad`、`type-number result=18 class=good`，因此 fixed-result CALL 后构造期 userdata/user value 可见根组合被证伪为当前 select-count 退化的直接缺口。
        ...
        - [x] 2026-07-07 修复：使用 `gopls check internal/native/capi_type_native.go internal/native/capi_type_native_test.go` 复核后，补齐 `lua_isuserdata` 导出入口；full userdata 与 lightuserdata 返回 true，nil、number、string、table、none 和无效 State 返回 false。`TestNativeCAPITypeAndNumberPrimitives` 已覆盖 full/light userdata 与非 userdata 边界。短 probe 仍为 `error-number result=12 class=bad`、`type-number result=18 class=good`，因此该 public C API 判断入口缺口需要修复，但不是当前 select-count 退化的充分根因。
...
- 2026-07-07：扩展 `scripts/check-native-skip-reasons.sh`，把 `scripts/test-native-luasocket.sh` 的 Windows runtime skip 文本纳入门禁；当前覆盖 Windows fixture / cjson / LPeg / LuaSocket 构建、LuaSocket Windows runtime 在 `lua53.dll` shim 或 import library 未落地前的 skip 文本，以及 Linux cross C compiler 缺失时 `scripts/check-native-cross-compile.sh` 的 skip 文本。该切口不把 Linux/Windows 运行期验收视为通过，只防止不可用平台被静默跳过。
...
- 2026-07-07：按二分/边界矩阵复核 `string-close-dead-branch-after-dead-pads` 诊断族；在上轮 Lua closure upvalue cell GC 扫描修复后，`pad=0..3` 的 `error-number` / `type-number` 均保持 `18`，`pad=4` 仅 `error-number` 退化到 `12`，`pad>=5` 两条路径均退化到 `12`。字节码常量表显示 `pad=4` 时预热 `"]"` 位于 `K(4)`、`select` 位于 `K(5)`，`pad=5` 时预热 `"]"` 位于 `K(5)`、`select` 位于 `K(6)`；因此旧的 `K(9)` / `K(10)` 判断过宽，后续应以 `pad=4/5` 的最小复现继续查通用构造期生命周期/临时根/常量闭包保活问题，而不是做 LPeg 或特定 pattern 的定向修复。
- 2026-07-07：新增 `string-close-dead-branch-after-dead-number-pads` / `lpeg-warmup-string-close-dead-branch-after-dead-number-pads-default-tail-error-number` 诊断模式，使用唯一大整数 padding 避免常量去重干扰，并与字符串 padding 对齐常量总数。macOS arm64 native probe 显示唯一 number padding 在 `pad=4` 同样使 `error-number` 从 `18` 退化到 `12`、`type-number` 保持 `18`，`pad=5` 两条路径均退化到 `12`；字节码常量表与字符串 padding 一样把 `"]"` / `select` 推到 `K(4)/K(5)`、`K(5)/K(6)`。因此“字符串对象生命周期”不是充分根因，后续应优先查常量表布局、预解码/inline cache、寄存器生命周期或 native 调用帧清理中依赖常量索引的通用缺口。
- 2026-07-07：使用已有 `fixed-result-global-value-*` perturbation 矩阵继续缩小范围；在唯一 number padding 下，`select-count-global-value-error-number` 与 `fixed-result-global-value-select-count-error-number` 均退化，`fixed-result-global-value-lua-return-select-count-error-number` 在 `pad=4` 保持 `18` 但 `pad=5` 退化到 `12`，`fixed-result-global-value-lua-return-two-error-number` 和 `fixed-result-global-value-builtin-rawget-error-number` 在 `pad=4/5` 均保持 `18`。该结果证伪“任意前置 builtin/固定返回都污染 LPeg 构造”，下一步应优先查顶层 `select("#", ...)`/vararg 返回处理后的调用帧、寄存器清理与常量布局组合，而不是做 LPeg 或某个 API 的特化修复。
- 2026-07-07：新增 `TestNativeLuaUserValueJoinTablesWithShiftedRawSetI` 模拟 LPeg `joinktables/concattable` 但保持模块无关，覆盖两个 userdata uservalue table 合并、新 ktable 通过 `lua_rawgeti`/`lua_rawseti` 写入、`lua_setuservalue` 挂回新 userdata 和临时 C frame 栈清理。该 native 定向测试通过，证伪“构造期 ktable 合并、rawseti 负索引修正或 setuservalue 弹栈”是当前 select-count 退化的直接缺口；下一轮继续转向 vararg/CALL 返回写回、常量布局和 native 可见根组合。
- 2026-07-07：新增固定返回 CALL 非结果实参槽清理边界；普通 Lua closure 与无 native C frame 的 Go 回调可在固定返回写回后把非结果实参槽清为 nil，活动 local 按 CALL 后 PC 保留，native C closure/`GoClosureWithUpvalues` 暂不清理。局部 guard 与 call_trace 已通过并确认 `select`/`rawequal` 的历史实参槽被清掉；但 LPeg select-count 矩阵仍保持 `select-count-global-value-error-number=12 bad`、`fixed-result-global-value-select-count-error-number=12 bad`、`fixed-result-global-value-builtin-rawequal-error-number=12 bad`、`fixed-result-global-value-builtin-rawget-error-number=18 good`。本轮同时证伪了无差别清理所有固定 CALL：该实验会把相关 LPeg probe 推成 `class=crash exit=2`，未保留。后续继续查 native C closure wrapper/root/GC 生命周期。
- 2026-07-07：新增 `TestNativeCFrameVisibleStackRootsWeakSweep`，把唯一强引用放入当前 C frame 可见临时栈，再触发 weak table sweep，确认该临时栈值会作为 `State.stack` root 保活 weak value。该 native 定向测试通过，证伪“native C API 临时栈值未进入 weak sweep 强根”是当前 select-count 退化的直接缺口；下一轮继续查 native C closure wrapper、构造期对象挂接和 registry/metatable/userdata 可见根组合。
- 2026-07-07：新增 runtime external GC root frame 机制，并让 native C function frame 的 upvalue 快照在调用期间同步登记为临时强根。默认门禁 `TestSweepWeakTablesKeepsExternalGCRootValues` 覆盖 external root 保活 weak value 和 pop 后清理，native 门禁 `TestNativeCFrameUpvaluesRootWeakSweep` 覆盖 C closure 调用期 upvalue 作为唯一可达根时不会被 weak sweep 误删。`CGO_ENABLED=0 go test ./...` 与 `CGO_ENABLED=1 go test -tags native_modules ./...` 均通过；短 probe 仍为 `error-number=12 bad` / `type-number=18 good`，说明该通用生命周期缺口已修复但不是当前 LPeg select-count 退化的充分根因。
- 2026-07-07：新增 `TestSweepWeakTablesKeepsRegistryAssociationValues`，覆盖 registry 中的命名元表 table raw metatable、registry 中 userdata 的 user value 和 raw metatable 关联边均能保活 weak value。该门禁只补测试不改生产代码，`CGO_ENABLED=0 go test ./...` 与 `CGO_ENABLED=1 go test -tags native_modules ./...` 均通过；因此 registry/metatable 关联边漏扫被证伪为当前 LPeg select-count 退化的直接缺口。
- 2026-07-07：新增 `TestNativeLuaCallKFixedResultKeepsConstructedUserdataRoots`，覆盖固定返回 `lua_callk` 写回后，同一 C frame 继续创建 native full userdata、构造 user value table，并由该 user value 间接保活 weak value。该门禁只补测试不改生产代码；短 probe 仍为 `error-number=12 bad` / `type-number=18 good`，因此 fixed-result CALL 后构造期 userdata/user value 可见根组合被证伪为当前 LPeg select-count 退化的直接缺口。
...
- 2026-07-07：补齐 `lua_isuserdata` 导出入口：full userdata 与 lightuserdata 返回 true，nil、number、string、table、none 和无效 State 返回 false。该修复提升通用 Lua 5.3 public C API 兼容性；短 probe 仍为 `error-number=12 bad` / `type-number=18 good`，因此不是当前 LPeg select-count 退化的充分根因。
- 2026-07-07：补齐 `lua_iscfunction` 导出入口：宿主侧 Go closure 与 native C closure wrapper 返回 true，Lua closure、带 `__call` 的 table、userdata、number、nil、none 和无效 State 返回 false。该修复提升通用 Lua 5.3 public C API 类型判断兼容性；短 probe 仍为 `error-number=12 bad` / `type-number=18 good`，因此不是当前 LPeg select-count 退化的充分根因。
- 2026-07-07：补齐 `lua_absindex` 导出入口：负索引按当前 C frame 可见栈顶规范化为稳定正索引，registry/upvalue pseudo-index 原样返回，正索引保持 C API 视角而不转换成 Go State 全局槽位。该修复提升通用 Lua 5.3 public C API 栈索引兼容性；短 probe 仍为 `error-number=12 bad` / `type-number=18 good`，因此不是当前 LPeg select-count 退化的充分根因。
- 2026-07-07：补齐 `lua_version` 导出入口：`L == NULL` 与有效 State 均返回同一个 Lua 5.3 `lua_Number(503)` 静态地址，满足 lauxlib 版本一致性检查。该修复提升通用 Lua 5.3 public C API 兼容性；短 probe 仍为 `error-number=12 bad` / `type-number=18 good`，因此不是当前 LPeg select-count 退化的充分根因。
- 2026-07-07：集中排查 LPeg 短 probe 的 `error-number=12` 时，先用 `PROBE_GC_BEFORE_TAIL=1` 复现到通用 GC 图遍历栈溢出；根因是 weak sweep 强引用收集、ephemeron 固定点传播、普通 weak sweep 与 finalizer 前 weak value sweep 在展开 Lua closure / 显式 Go closure upvalue 时只记录 table 访问集，未记录 closure 访问集，遇到闭包自引用 upvalue 会无限递归。已在 `runtime/gc.go` 为上述路径统一增加 closure visited 集合，并新增 `TestSweepWeakTablesHandlesSelfReferentialLuaClosureUpvalues`、`TestSweepWeakTablesHandlesSelfReferentialGoClosureUpvalues`、`TestSweepWeakValuesBeforeFinalizersHandlesSelfReferentialLuaClosureUpvalues`。复跑 `PROBE_GC_BEFORE_TAIL=1` 不再崩溃，但 `error-number`、`type-number`、`rawget-number` 在 GC-before-tail 下均为 `12 bad`；复跑原始短矩阵仍为 `error-number=12 bad`、`type-number=18 good`、`rawequal-number=12 bad`、`rawget-number=18 good`。因此本轮修复的是通用 GC 自环卡点，原始 LPeg select-count 退化仍需继续沿“常量/寄存器布局、固定返回 CALL、native pattern 构造期根”方向定位。
- 2026-07-07：继续集中排查原始 LPeg 短 probe 的 `error-number=12`，本轮不改生产代码，改用现有矩阵做通用边界证伪。`select-count-nonempty`、`select-count-two-numbers`、`select-count-table-constructor`、`select-count-multivar`、`select-count-inline-if` 均退化为 `12 bad`，而 `lua-return-select-count-nonempty`、`lua-return-integer-two-after-vararg`、`builtin-tonumber-string-base`、`builtin-rawget-table-string`、`builtin-assert-*` 保持 `18 good`，说明 `select("#", ...)` 的返回值计算本身不是错点，问题更偏向顶层主 frame 内 CALL 后状态与后续构造/匹配组合。`select-count-global-only-error` 保持 `18 good`，但 `select-count-global-message-error`、`select-count-value-global-number-error`、`select-count-holder-table-error-number` 退化为 `12 bad`；同类 `type/rawget/tonumber` 对照保持 `18 good`，`rawequal` 在带 message/value 组合时也可退化。`select-count-error-message-do-block`、`select-count-error-message-clear-nil`、`select-count-error-message-clear-false`、`select-count-nil-after-use`、`select-count-overwrite-args-*` 均仍为 `12 bad`，证伪“只要清掉 Lua local、缩小 block 或覆盖 CALL 参数槽即可恢复”。下一轮应优先查活动 Lua VM 寄存器快照、PC 生命周期区间、Go closure 强根集合与 LPeg 构造期弱表/ktable 交互，而不是继续做 LPeg 特化或无差别 CALL 槽清理。
- 2026-07-07：用纯 Lua 元方法复现和 Delve 断点确认 `error-number=12` 的通用 codegen 根因，不再依赖 LPeg/native。最小复现中当 padding 使 `"]"` 从 `K(255)` 变为 `K(256)` 后，`compileBinaryLeftRKRightToTargetTo` 在 `"]" * m.C(...)` 处通过 `safeRKOperand` 把左字面量装入临时寄存器 `R6`，随后右侧复杂表达式继续编译并复用/覆盖该寄存器，运行时首个 `__mul` 收到的左操作数从 `"]"` 变为 `0`。Delve 断点确认命中位置为 `operator="*"`, `targetRegister=4`, `nextRegister=7`, `leftOperand=6`, 左侧 AST 为 string literal `"]"`、右侧 AST 为 `m.C(...)` 调用；因此根因是 `safeRKOperand` 实际可能分配溢出 RK 常量临时寄存器，却没有把该寄存器生命周期返回给调用方。修复前必须统一审计并调整所有 `safeRKOperand` 调用点，尤其是二元表达式、self binary、return binary 和固定返回 CALL 参数槽回收路径，避免只修 LPeg 暴露出的单一路径。
- 2026-07-07：修复 `safeRKOperand` 隐式临时寄存器生命周期缺口；该 helper 现在显式返回可能分配的寄存器，二元 RK 快路径、self binary 右操作数、可复用目标右操作数等调用点在发射使用指令后释放。新增不会分配临时寄存器的 `safeInlineRKOperand`，让 `compileBinaryLeftRKRightToTargetTo` 在左操作数为溢出 RK 常量时回退通用路径，避免右侧复杂表达式覆盖左字面量。新增纯 Lua 回归 `TestDoStringRKOverflowBinaryLeftLiteralSurvivesRightCall`；复跑 LPeg 短 probe 后 `select-count-global-value-error-number` 与 `select-count-global-value-type-number` 均恢复为 `result=18 class=good`。
- 2026-07-07：修复 native `lua_callk` 调用 Go closure 的负载覆盖缺口；`nativeLuaCallGoClosure` 现在支持 `runtime.GoUnaryFunction` 与 `*runtime.GoFastUnaryFunction`，保持只消费首个实参、多余实参按 Lua 调用语义忽略。新增 `TestNativeLuaCallKCallsUnaryGoClosureShapes`，在当前 C frame 内覆盖普通一元 Go closure 与 fast unary closure，避免 LPeg match-time capture 调用 `math.sin` 这类标准库一元函数时被误判为不可调用。
