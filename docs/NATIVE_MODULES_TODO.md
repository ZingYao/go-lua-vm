  ...
  - [ ] `lpeg` 或等价纯 C 模块验收：覆盖 userdata、metatable、registry 和复杂 C function 行为。
    ...
    - [ ] 完整 `third_party/lpeg/test.lua`、复杂 capture、grammar 和错误边界仍待后续验收。
      ...
      - [ ] 当前完整测试推进到 1159 行；同一 pattern 独立运行返回 18，但完整前序状态后返回 12，疑似 LPeg match-time capture / named capture 前序状态污染，需继续缩小最小复现。
        ...
        - [ ] 下一轮继续使用 gopls 审查 native/LPeg 前序状态后顶层直接 `select("#", 两个以上 string/number 变参)` 的固定单返回、后续写入 `error` 函数值和消息字符串、以及 `CALL C=0` 开放返回接 `SETLIST` 后，VM 历史寄存器内容、openTop、返回值写回、调用帧清理和 native `lua_call` 可见栈边界是否存在通用语义缺口；同时优先用指定模式 probe 快速对照无 `select` good、`type+message` good、`error+message` bad、清空仍 bad、`SETLIST` bad，只有能抽象为通用 VM/native 调用帧隔离语义时才进入生产代码小切口。
