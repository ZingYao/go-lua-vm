  ...
  - [ ] `lpeg` 或等价纯 C 模块验收：覆盖 userdata、metatable、registry 和复杂 C function 行为。
    ...
    - [ ] 完整 `third_party/lpeg/test.lua`、复杂 capture、grammar 和错误边界仍待后续验收。
      ...
      - [ ] 当前完整测试推进到 1159 行；同一 pattern 独立运行返回 18，但完整前序状态后返回 12，疑似 LPeg match-time capture / named capture 前序状态污染，需继续缩小最小复现。
        ...
        - [ ] 下一轮继续使用 gopls 审查 native/LPeg 前序状态后顶层直接 `select("#", 两个以上 string/number 变参)` 的 `CALL C=2` 固定结果写回、`CALL C=0` 开放返回写回、`baseLuaCallArguments`/`Select` vararg 参数切片、后续 `MOVE/EQ/TEST` 或 `SETLIST` 消费路径、active VM 根扫描和 native `lua_call` 栈隔离；同时优先用指定模式 probe 快速对照三槽清理仍 bad、直接丢弃 good、GC 时机仍 bad、`rawequal`/`tonumber` good 组，只有能抽象为通用 VM/native 调用帧隔离语义时才进入生产代码小切口。
