  ...
  - [ ] `lpeg` 或等价纯 C 模块验收：覆盖 userdata、metatable、registry 和复杂 C function 行为。
    ...
    - [ ] 完整 `third_party/lpeg/test.lua`、复杂 capture、grammar 和错误边界仍待后续验收。
      ...
      - [ ] 当前完整测试推进到 1159 行；同一 pattern 独立运行返回 18，但完整前序状态后返回 12，疑似 LPeg match-time capture / named capture 前序状态污染，需继续缩小最小复现。
        ...
        - [ ] 下一轮继续使用 gopls 审查 native/LPeg 前序状态后顶层直接 `select("#", 两个以上 string/number 变参)` 的固定单返回、后续写入特定 Go 函数值与 string/number 标量载荷、以及扰动后构造 LPeg pattern 时，VM 活动寄存器根集合、函数值 identity、openTop、返回值写回、调用帧清理、native `lua_call` 可见栈边界、registry/user value/ktable 写入是否存在通用语义缺口；同时优先用指定模式 probe 快速对照无 `select` good、`type/assert/rawget+string` good、`error/next/rawequal+string|number` bad、`error+boolean|nil|table|function` good、函数-only good、payload/function 反向仍 bad、spacer 仍 bad、清空仍 bad、只预构造 close/head/back/function 任一即恢复、只预构造 dummy func/capture/back 也恢复、只预构造 open/any 不恢复，并继续区分 table/string/number 等 dummy 对象；只有能抽象为通用 VM/native 调用帧隔离或 GC 根语义时才进入生产代码小切口。
