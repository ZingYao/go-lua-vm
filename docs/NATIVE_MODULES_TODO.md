  ...
  - [ ] `lpeg` 或等价纯 C 模块验收：覆盖 userdata、metatable、registry 和复杂 C function 行为。
    ...
    - [ ] 完整 `third_party/lpeg/test.lua`、复杂 capture、grammar 和错误边界仍待后续验收。
      ...
      - [ ] 当前完整测试推进到 1159 行；同一 pattern 独立运行返回 18，但完整前序状态后返回 12，疑似 LPeg match-time capture / named capture 前序状态污染，需继续缩小最小复现。
        ...
        - [ ] 下一轮基于上述模块无关门禁评估最小生产修复：仅允许清理 fixed-result 普通 CALL 的非结果临时实参槽，并必须显式排除 `CALL C=0` 开放返回、`TAILCALL`、`TFORCALL`、仍活动 local、debug hook/traceback 可见帧和 `__call` 改写参数区间；若任一条件无法证明安全，则继续比较后续 `EQ/LOADBOOL/TEST/JMP`、`skipped/payload` 写入阶段和 active register/native 可见根是否需要收缩。
