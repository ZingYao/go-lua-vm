...
| Go reflection 自动绑定是否进入首版 | 不进入首版；reflection 自动绑定指“自动扫描 Go struct/function 并暴露字段、方法到 Lua”，当前只承诺显式注册 Go 函数、显式 table/module/object 绑定和 stub 生成 | 显式绑定已实现并可测试；自动反射会扩大可见 API、权限和错误边界 |
