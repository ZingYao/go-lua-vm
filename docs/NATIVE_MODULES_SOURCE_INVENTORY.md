...
| 类别 | 目标路径 | 用途 | 阻塞影响 |
| --- | --- | --- | --- |
| LPeg 构建/运行期脚本 | 待定 `scripts/build-native-lpeg.sh` / `scripts/test-native-lpeg.sh` | 使用仓库内 `third_party/lpeg/` 和 Lua 5.3 public headers 编译并运行第二真实模块验收 | 复杂 C API 兼容面仍未运行期闭环 |
...
- `third_party/lpeg/` 当前只完成源码固定和来源/许可记录；尚未声明 LPeg 编译或运行期通过，后续必须新增独立构建和 `require("lpeg")` 验收脚本后才能闭环该真实模块。
