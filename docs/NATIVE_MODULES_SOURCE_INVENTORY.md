...
- `lua-cjson` 真实模块源码当前可通过 `CGO_ENABLED=1 scripts/build-native-cjson.sh` 做源码编译级验收；这只证明仓库内 headers 和固定源码可构建动态模块，不能替代 `require("cjson")`、`encode/decode` 和错误输入 `pcall` 的运行期验收。
