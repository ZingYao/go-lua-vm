...
| Skip 原因检查脚本 | `scripts/check-native-skip-reasons.sh` | 验证 Windows shim 未落地、LuaSocket/真实模块总验收 Windows runtime 暂不可用、真实模块总验收非 Windows 异平台误用和缺失 cross C compiler 等场景必须输出明确 `skip:` 原因，防止平台不可用被静默视为通过 | 已固定 |
