...
| 真实模块构建脚本 | `scripts/build-native-luasocket.sh` | 使用仓库内 Lua 5.3 public headers 和固定 `third_party/luasocket/` 源码编译 `socket/core` 与 `mime/core` 动态模块；Linux 产出 `.so`，macOS 产出 `.so` / `.dylib`，Windows 在显式 C compiler 和 Lua import library 齐备时用 `wsocket.c` / `ws2_32` 产出 `.dll`；脚本显式输出宿主/目标平台、`CC`、源码路径和产物路径，缺少 Windows 输入时明确 skip | 已固定 |
