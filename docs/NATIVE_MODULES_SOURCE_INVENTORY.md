...
| Skip 原因检查脚本 | `scripts/check-native-skip-reasons.sh` | 验证 Windows shim 未落地、LuaSocket Windows runtime 暂不可用和缺失 cross C compiler 等场景必须输出明确 `skip:` 原因，防止平台不可用被静默视为通过 | 已固定 |
...
| 真实模块总验收脚本 | `scripts/test-native-real-modules.sh` | 串联 fixture、lua-cjson、LPeg 和 LuaSocket 当前平台运行期验收，统一输出目标平台和 CGO 状态；Linux/Windows 仍由各子脚本和目标平台闭环单独确认 | 已固定 |
...
- `scripts/test-native-real-modules.sh` 可作为当前平台真实模块总验收入口，串联 fixture、lua-cjson、LPeg 和 LuaSocket 运行期脚本；该入口只聚合当前平台结果，不把缺失目标平台的 skip 解释为 Linux/Windows 已闭环。
