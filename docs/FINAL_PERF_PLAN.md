...
截至 2026-07-05，最终性能收敛分支已经完成可验证的小切口：

- `compile_3000_functions` 从最终分支起点约 `1.31x` 收敛到约 `1.22x`；Go micro 从约
  `2.65-2.71 ms/op` 降到当前约 `2.26-2.35 ms/op`，B/op 从约 `3.78 MB` 降到约 `3.50 MB`。
- `table_rw`、`closure_upvalue`、`arith_chain_temp`、`arith_add_loop`、`stdlib_math_string`
  已稳定快于官方 Lua 5.3.6。
- `recursion`、`function_call`、`string_concat`、`arith_mix_loop` 未稳定超过各自进入门槛，只能继续观察。

最终判断：当前分支不再有符合“小切口、可回退、能证明语义等价、能通过 benchmark 门槛”的生产优化项。
后续重新打开必须满足至少一项新触发条件：

- 默认完整 benchmark 三轮显示某运行期项目稳定超过文档门槛。
- `compile_3000_functions` 出现新的结构性设计，能够减少整段顶层函数声明对象数量或重构 direct child
  Proto 生命周期，同时完整证明 debug/luac/错误位置语义等价。
- Go 编译器/runtime 升级后 profile 形态发生实质变化，出现新的源码侧单一主因。
