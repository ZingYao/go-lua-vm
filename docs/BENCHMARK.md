...
- 按最新完整 benchmark 复核口径，`compile_3000_functions` 和 `stdlib_math_string` 稳定低于 3x；`table_rw` 贴近但低于 3x，prepared 口径确认其 B/op 主要来自运行期 table 数组区扩容；`recursion` 经 `b24f6c0` 后在抽样完整 benchmark 中回到 3x 以下，但仍需默认三轮复核；`arith_chain_temp` 仍是执行 CPU 侧的边缘观察项。
