...
- 按最新完整 benchmark 复核口径，`compile_3000_functions` 和 `stdlib_math_string` 稳定低于 3x；`table_rw` 贴近但低于 3x，prepared 口径确认其 B/op 主要来自运行期 table 数组区扩容；`recursion` 与 `arith_chain_temp` 三轮略高于 3x，下一轮应作为优先定位对象。
