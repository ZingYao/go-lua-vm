...
`033b05a` 又新增预编译执行口径，用同一个 State 预先 `LoadString` 顶层 closure 后反复 `Call`，剥离
源码编译、OpenLibs 和 State 初始化。`BenchmarkPreparedArithChainTempOfficial` 三轮为
`34.48 / 34.37 / 34.21 ms/op`、约 `22 B/op`、`0 allocs/op`，与端到端
`BenchmarkDoStringArithChainTempOfficial` 的 `34.46 / 34.59 / 34.32 ms/op` 基本一致，说明
`arith_chain_temp` wall-clock 几乎全是执行循环 CPU。`BenchmarkPreparedRecursion` 三轮为
`7.32 / 7.21 / 7.22 ms/op`、约 `546 B/op`、`6 allocs/op`，与端到端
`BenchmarkDoStringRecursion` 的 `7.21 / 7.17 / 7.17 ms/op` 同量级。

继续 profile `BenchmarkPreparedRecursion` 的 alloc_objects 后，稳定分配主要落在
`runtime.NewLuaClosure`、`VM.executeClosure`、`runtime.NewTable`、`runtime.luaProtoLeafAddReturn` 和
`VM.BindPrototype`。其中 `runtime.NewLuaClosure` 来自每次执行顶层 chunk 时创建局部函数 `fib` closure，
这是源码语义的一部分；`BindPrototype` 和 leaf-add-return 元数据与 VM/proto 运行期绑定相关。当前没有
发现可直接删除的临时分配，后续不应把这 6 allocs/op 误判为编译或 OpenLibs 残留分配。

`d062487` 随后优化 `runtime.NewLuaClosure`：先计算 `luaProtoDirectCallSafe`，只有 direct-safe
Proto 才继续预解析 `LeafAddReturn` 与 `LeafUpvalueAddSetReturn`。两类 caller-side 叶子快路径形态
都不包含 `CALL`、`TAILCALL`、`TFORCALL` 或 `CLOSURE`；一旦 direct-call-safe 因嵌套调用或闭包创建
失败，这些叶子形态必然无法命中。因此该改动只减少非叶子 closure 创建期的 Proto 形态扫描，不扩大
direct Lua CALL 覆盖面，也不改变 debug hook、coroutine、traceback 或错误路径语义。复核中
`BenchmarkPreparedRecursion` 从 6 allocs/op 降到稳定 5 allocs/op，`BenchmarkDoStringRecursion`
降到 361 allocs/op；wall-clock 仍主要受 Lua CALL 边界影响。

`e03edaa` 补齐 `arith_add_loop` 和 `arith_mix_loop` 的预编译执行口径。`BenchmarkPreparedArithAddLoopOfficial`
三轮为 `16.30 / 16.30 / 16.29 ms/op`、约 `9 B/op`、`0 allocs/op`，与
`BenchmarkDoStringArithAddLoopOfficial` 的 `16.38 / 16.38 / 16.38 ms/op` 基本一致。
`BenchmarkPreparedArithMixLoopOfficial` 三轮为 `28.16 / 28.15 / 28.18 ms/op`、约 `20 B/op`、
`0 allocs/op`，与 `BenchmarkDoStringArithMixLoopOfficial` 的 `28.19 / 28.20 / 28.18 ms/op`
基本一致。由此可见，`arith_add_loop`、`arith_mix_loop` 和 `arith_chain_temp` 的完整耗时几乎都是
纯执行 CPU；编译、OpenLibs 和 State 初始化主要贡献分配，不贡献明显 wall-clock。

`0a93305` 补齐 `table_rw` 的预编译执行口径。`BenchmarkPreparedTableReadWriteOfficial` 三轮为
`13.91 / 13.07 / 12.66 ms/op`、约 `33.50 MB/op`、`18 allocs/op`，相比
`BenchmarkDoStringTableReadWriteOfficial` 的 `15.13 / 15.06 / 14.83 ms/op`、
约 `33.56 MB/op`、`262 allocs/op`，只剥离了编译和初始化带来的对象数量，B/op 仍保持约 33.5 MB。
这与此前 `table_rw` profile 中 `runtime.(*Table).ensureArraySize` 占 alloc_space 约 `99.78%`
一致，说明主要成本是每次执行创建 table 并连续扩容数组区，而不是加载或编译噪声。后续若继续优化
`table_rw`，必须找到新的 Lua 5.3 语义等价预估或预分配入口；不应重复数组扩容阈值调参。

`b24f6c0` 优化 `OP_CLOSURE` 的单 upvalue 捕获路径：当子 Proto 只有 1 个 upvalue 时，closure
直接使用内嵌的单元素 upvalue 快照和共享 cell 槽，避免为 `[]Value` 与 `[]*UpvalueCell` 各分配一段
切片底层数组。该路径只覆盖 VM 执行 `OP_CLOSURE` 自己创建的 closure，不改变 `NewLuaClosure` 对外传入
slice 的别名行为，也不改变 debug.getupvalue、debug.setupvalue、upvaluejoin、yield 或 traceback 语义。
Go micro 复核中，`BenchmarkPreparedRecursion` 从 `5 allocs/op` 降到 `3 allocs/op`，
`BenchmarkDoStringRecursion` 从 `361 allocs/op` 降到 `359 allocs/op`，`BenchmarkDoStringFunctionCall`
小幅降到 `250 allocs/op`。`BenchmarkDoStringArithChainTempOfficial` 仍为约 `33.65-33.82 ms/op`、
`220 allocs/op`，未显示算术链退化。

基于 `b24f6c0` 重建 `bin/glua` / `bin/gluac` 后，用正确 Lua 5.3.6 工具跑抽样完整 benchmark
（`BENCH_ITERATIONS=20`、`BENCH_COMPILE_ITERATIONS=15`、`BENCH_WARMUP_ITERATIONS=3`）结果如下：

| 用例 | 官方工具中位数 | 本项目中位数 | 本项目/官方 |
| --- | ---: | ---: | ---: |
| `arith_add_loop` | 0.008046s | 0.022064s | 2.74x |
| `arith_mix_loop` | 0.011981s | 0.034715s | 2.90x |
| `arith_chain_temp` | 0.013749s | 0.041040s | 2.98x |
| `table_rw` | 0.007837s | 0.022384s | 2.86x |
| `function_call` | 0.007484s | 0.022672s | 3.03x |
| `string_concat` | 0.006493s | 0.009498s | 1.46x |
| `closure_upvalue` | 0.008971s | 0.021922s | 2.44x |
| `stdlib_math_string` | 0.020411s | 0.038259s | 1.87x |
| `recursion` | 0.004631s | 0.013169s | 2.84x |
| `compile_3000_functions` | 0.005912s | 0.016276s | 2.75x |

该抽样中 `recursion` 回到 3x 以下，但官方基线也明显慢于上一轮默认完整 benchmark，因此只能说明
`b24f6c0` 后没有观察到递归退化，不能单独替代三轮默认完整复跑。`function_call` 单轮为 `3.03x`，
同样需要后续用 Go micro 或默认完整复跑确认是否只是官方基线波动。

继续按默认完整口径复跑三轮后，结果如下：

| 用例 | 官方工具中位数 | 本项目中位数 | 本项目/官方 |
| --- | ---: | ---: | ---: |
| `arith_add_loop` | 0.008149s / 0.008442s / 0.008172s | 0.022102s / 0.022623s / 0.022355s | 2.71x / 2.68x / 2.74x |
| `arith_mix_loop` | 0.011947s / 0.012261s / 0.012158s | 0.034415s / 0.035316s / 0.034602s | 2.88x / 2.88x / 2.85x |
| `arith_chain_temp` | 0.013587s / 0.014120s / 0.013734s | 0.040367s / 0.041397s / 0.040661s | 2.97x / 2.93x / 2.96x |
| `table_rw` | 0.007489s / 0.007879s / 0.007753s | 0.022119s / 0.022432s / 0.022131s | 2.95x / 2.85x / 2.85x |
| `function_call` | 0.007557s / 0.007646s / 0.007490s | 0.019501s / 0.019483s / 0.019274s | 2.58x / 2.55x / 2.57x |
| `string_concat` | 0.006141s / 0.005361s / 0.005356s | 0.009780s / 0.009774s / 0.009607s | 1.59x / 1.82x / 1.79x |
| `closure_upvalue` | 0.008777s / 0.008853s / 0.008800s | 0.021955s / 0.028368s / 0.021872s | 2.50x / 3.20x / 2.49x |
| `stdlib_math_string` | 0.020819s / 0.023594s / 0.020052s | 0.039161s / 0.038926s / 0.038681s | 1.88x / 1.65x / 1.93x |
| `recursion` | 0.004424s / 0.004406s / 0.004243s | 0.012657s / 0.014346s / 0.012384s | 2.86x / 3.26x / 2.92x |
| `compile_3000_functions` | 0.005980s / 0.007232s / 0.005833s | 0.013360s / 0.016981s / 0.013051s | 2.23x / 2.35x / 2.24x |

默认三轮确认 `function_call` 回到 2.55x-2.58x，抽样中的 `3.03x` 是单轮波动。`recursion`
仍有一轮慢到 `3.26x`，但另外两轮为 `2.86x` / `2.92x`，已不再像上一轮默认完整复核那样三轮稳定
