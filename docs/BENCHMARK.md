...
- CLI 冷启动和小脚本差距较小，历史冷启动约 1.25x 到 1.35x；本轮 `compile_3000_functions` 为 2.65x / 2.56x / 2.53x，仍低于当前 3x 目标线。
- 按当前完整 benchmark 复核口径，暂无线性稳定高于 3x 的路径；`arith_chain_temp`、`arith_mix_loop`、`table_rw`、`arith_add_loop` 与 `recursion` 仍贴近 3x，需要继续作为边缘回归观察项；`function_call`、`closure_upvalue` 与 `stdlib_math_string` 低于 3x，仍需回归观察。
