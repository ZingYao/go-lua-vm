...
- LuaSocket 或等价网络库：放在平台闭环后段，用于验证系统依赖和 socket 行为；当前先提供源码编译入口，运行期 loopback 验收仍是独立 TODO。
...
当前 `scripts/build-native-luasocket.sh` 可在 macOS arm64 上使用仓库内 LuaSocket v3.1.0 源码和 `native/lua53/include/` 编译 `socket/core.so`、`socket/core.dylib`、`mime/core.so` 和 `mime/core.dylib`。该脚本只证明源码编译闭环，`require("socket")`、`require("mime")` 与本机 TCP/UDP loopback 行为仍等待运行期脚本闭环。
