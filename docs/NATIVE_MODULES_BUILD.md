...
当前 `native_modules` 已经能在 macOS arm64 上用仓库内 `lua-cjson` 源码完成 `.so` 与 `.dylib` 两种后缀的运行期验收。验收覆盖 `require("cjson")`、`encode/decode`、`cjson.null`、非法 JSON `pcall` 和不可序列化 function `pcall`。
...
- 现成官方 Lua 5.3 ABI 二进制模块仍未闭环；Linux/macOS 还需验证宿主符号导出或 `liblua5.3` / `liblua` 兼容 shim，Windows 还需 `lua53.dll` shim 或等价 import library。
