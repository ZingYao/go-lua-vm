...
该入口使用仓库内 Lua public headers、固定 `third_party/luasocket/` 源码、Windows socket backend、`ws2_32` 和 Windows Lua import library 构建 `socket/core.dll` 与 `mime/core.dll`。缺少 Windows C compiler 或 import library 时会明确 `skip:`；它只覆盖真实模块源码构建，不代表 Windows `require("socket")`、`require("mime")` 或 TCP/UDP loopback 运行期验收已通过。
...
当前 `scripts/build-native-luasocket.sh` 可在 macOS arm64 上使用仓库内 LuaSocket v3.1.0 源码和 `native/lua53/include/` 编译 `socket/core.so`、`socket/core.dylib`、`mime/core.so` 和 `mime/core.dylib`；Windows 目标在显式 C compiler 和 Lua import library 可用时提供 `socket/core.dll` 与 `mime/core.dll` 源码构建入口。`scripts/test-native-luasocket.sh` 会构建 native `glua` 与 LuaSocket 模块，并分别按当前平台后缀验证 `require("mime")`、基础 MIME 编解码、`require("socket")`、`socket.tcp()` / `socket.udp()` 以及本机 TCP/UDP loopback；Windows 运行期仍等待目标平台闭环。
