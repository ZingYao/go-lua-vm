...
## 9eb7f54 后 `recursion` profile 复核

2026-07-06 在 `function_call` 已降到三轮 `0.82x` 后，按剩余排序重新复核 `recursion` prepared micro/profile。
五轮 `BenchmarkPreparedRecursion` 为 `1706 / 1706 / 1707 / 1700 / 1703 ns/op`、`0 B/op`、`0 allocs/op`。
单次 profile benchmark 为 `1730 ns/op`；`GOGC=off` 为 `1727 ns/op`，同样保持 `0 B/op`、`0 allocs/op`。
...
- memory profile 不再出现执行期 `runtime.NewLuaClosure` 或 `runtime.NewOpenUpvalueCell`；剩余可见分配来自
  profile/test harness、一次性编译和 Go runtime 背景分配，不对应 `b.ReportAllocs` 的 prepared 执行期分配。

结论：borrowed closure 已把上一轮明确主因 `224 B/op`、`2 allocs/op` 全部消除；当前剩余差距主要是
prepared 执行框架、VM step 和已有自递归整数 fib fast path 的固定 CPU。继续做同一方向的 closure/upvalue 生命周期
扩张没有收益证据；若要把 `recursion` 继续压到 `1.00x` 以下，只剩更激进的整段 benchmark-specific 递归折叠或
调用帧绕过，这会影响 debug hook、coroutine/yield、traceback、错误 PC、调用帧可见性和普通递归语义，因此本轮
...
## `arith_mix_loop` PC 预筛

2026-07-06 在 `031fe31` 后复核官方规模 `arith_mix_loop` micro/profile。五轮 DoString 为
`7.532-7.551 ms/op`、约 `130.6 KB/op`、`199 allocs/op`；五轮 prepared 为 `7.497-7.513 ms/op`、
`66 B/op`、`0 allocs/op`。DoString 与 prepared 基本重合，说明剩余差距不在编译/OpenLibs；prepared 路径无分配，
也没有新的 GC 切口。

CPU profile 显示可见主因集中在已有执行框架和 mix batch：

- 普通 CPU：`executePreparedLuaClosureWithDebugNameTailFromArgs` flat `22.75%` / cum `96.41%`；
  `PrepareMixArithmeticForLoopBatch` flat `20.96%`；`TryExecuteMixArithmeticForLoopBatch` flat `20.36%` /
  cum `26.35%`；`TryExecuteMixArithmeticForLoop` flat `7.78%`；`integerFloorDiv` flat `5.39%`。
- `GOGC=off` CPU：`executePreparedLuaClosureWithDebugNameTailFromArgs` flat `24.42%` / cum `95.93%`；
  `TryExecuteMixArithmeticForLoopBatch` flat `20.93%` / cum `28.49%`；`PrepareMixArithmeticForLoopBatch`
  flat `14.53%`；`TryExecuteMixArithmeticForLoop` flat `11.05%`；`integerFloorDiv` flat `7.56%`。
- alloc profile 只剩 pprof/test harness 与 Go runtime 背景分配，不对应 prepared 执行期 `b.ReportAllocs`。

生产切口：为 mix superinstruction 增加 `HasMixArithmeticForLoopAt(pc)`，并让 `lua` API 热循环像其他
superinstruction 一样，只有当前 PC 已静态匹配 `MUL; ADD; SUB; IDIV; MOD; ADD; FORLOOP` 时才进入
`PrepareMixArithmeticForLoopBatch` / 单轮 fallback。该 helper 只读取已准备的表，不改变 batch 内动态 integer
guard、除数防御 guard、context 窗口、hook/coroutine 回退或错误路径；非目标 PC 直接跳过，避免每条普通指令都
承担 mix batch 准备成本。
...
| 指标 | PC 预筛前 | PC 预筛后 | 改善 |
| --- | ---: | ---: | ---: |
| `BenchmarkDoStringArithMixLoopOfficial` wall-clock | `7.532-7.551 ms/op` | `5.197-5.208 ms/op` | 中位数约下降 `31%` |
| `BenchmarkDoStringArithMixLoopOfficial` B/op | 约 `130.6 KB/op` | 约 `130.6 KB/op` | 持平 |
| `BenchmarkDoStringArithMixLoopOfficial` allocs/op | `199` | `199` | 持平 |
| `BenchmarkPreparedArithMixLoopOfficial` wall-clock | `7.497-7.513 ms/op` | `5.147-5.151 ms/op` | 中位数约下降 `31%` |
| `BenchmarkPreparedArithMixLoopOfficial` B/op | `66 B/op` | `45 B/op` | 小幅下降 |
| `BenchmarkPreparedArithMixLoopOfficial` allocs/op | `0` | `0` | 持平 |

重建 CLI 后跑三轮默认完整官方 benchmark，按三轮中位数计算：

| 排名 | English case | 中文名称 | 官方三轮中位数 | 本项目三轮中位数 | 本项目/官方 | 相对初始倍率 |
| ---: | --- | --- | ---: | ---: | ---: | ---: |
| 1 | `recursion` | 递归 | 0.003505s | 0.003695s | 1.054x | 改善约 0.026x，语义证伪 |
| 2 | `table_rw` | 表读写 | 0.006929s | 0.006035s | 0.871x | 已低于 1.0 |
| 3 | `closure_upvalue` | 闭包 upvalue | 0.007892s | 0.006725s | 0.852x | 已低于 1.0 |
| 4 | `arith_mix_loop` | 混合算术循环 | 0.011024s | 0.009175s | 0.832x | 改善约 0.178x |
| 5 | `string_concat` | 字符串拼接 | 0.004590s | 0.003778s | 0.823x | 已低于 1.0 |
| 6 | `function_call` | 函数调用 | 0.006610s | 0.005256s | 0.795x | 已低于 1.0 |
| 7 | `compile_3000_functions` | 编译3000个函数 | 0.005015s | 0.003879s | 0.773x | 已低于 1.0 |
| 8 | `arith_chain_temp` | 算术临时链 | 0.012395s | 0.009627s | 0.777x | 已低于 1.0 |
| 9 | `arith_add_loop` | 整数累加循环 | 0.007571s | 0.004865s | 0.643x | 已低于 1.0 |
| 10 | `stdlib_math_string` | 标准库数学与字符串 | 0.018876s | 0.010797s | 0.572x | 已低于 1.0 |

结论：`arith_mix_loop` 从上一轮三轮 `1.051x` 降到 `0.832x`，已稳定低于 `1.00x`。最终剩余高于 `1.00x`
的只有 `recursion`，而 `recursion` 已在 `9eb7f54` 后 profile 复核中证明 prepared 路径 `0 allocs/op`，
剩余 CPU 属于执行框架、VM step 和自递归 fast path 固定成本；继续优化需要整段递归折叠或调用帧绕过，会破坏
debug hook、coroutine/yield、traceback、错误 PC 和调用帧可见性，按语义门禁不进入生产实现。因此本轮专项达到
“所有剩余 >1.00x 项均已被 profile/语义门禁证伪且无安全生产切口”的完成条件。
