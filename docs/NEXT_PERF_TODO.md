...
2026-07-04 试验了最小“parser 摘要 + codegen 子 Proto 直发”方案：在完整 AST 构造后为
`return <param> + <integer>` 记录摘要，普通 `function name` 语句在摘要与 AST 二次校验一致时跳过
`compileBlock` / `compileReturn` 的通用子函数路径，直接发出 `ADD; RETURN`。该方案不跳过 lexer/parser，
也不减少 `NameExpression`、`LiteralExpression`、`BinaryExpression` 或 `ReturnStatement` AST 构造。
五轮 Go micro 结果为约 `2.637-2.672 ms/op`、`3.879 MB/op`、`89-90 allocs/op`；回退生产改动后
同口径复跑为约 `2.626-2.639 ms/op`、`3.781 MB/op`、`89 allocs/op`。该试验没有证明 wall-clock
收益，且 B/op 上升约 `98 KB/op`。生产改动已回退，只保留 bytecode/debug 定向测试和本证伪记录；
后续不要重复做“只在 codegen 阶段直发简单函数体”的小切口，除非方案能同时减少 parser AST 构造或
显著降低 B/op。

2026-07-04 在 `b2233be` 后按最新 HEAD 再次复核 `compile_3000_functions`：

```bash
CGO_ENABLED=0 go test ./internal/luac -run '^$' \
  -bench '^BenchmarkCompileSource3000Functions$' \
  -benchmem -benchtime=5s -count=1 \
  -cpuprofile /tmp/go-lua-vm-next-profiles/compile_3000_after_b2233be_cpu.pprof \
  -memprofile /tmp/go-lua-vm-next-profiles/compile_3000_after_b2233be_mem.pprof
CGO_ENABLED=0 go test ./internal/luac -run '^$' \
  -bench '^BenchmarkCompileSource3000Functions$' \
  -benchmem -benchtime=3s -count=5
```

- 单轮 profile：约 `2.628 ms/op`、`3.78 MB/op`、`85 allocs/op`。
- 五轮 micro：约 `2.585-2.591 ms/op`、`3.78 MB/op`、`87 allocs/op`。
- CPU profile 仍被 Go runtime/GC/调度采样噪声主导；源码侧可归因热点分散在
  `Lexer.NextToken`、`Parser.advance`、`parseBlockUntilInto`、`parseFunctionStatement`、
  `parseFunctionBodyInto` 和 `compileBlock`，没有新的独立 flat 主热点。
- `alloc_space` 仍主要来自 `newFunctionStatement`、`prepareDirectFunctionBlockCapacity`、
  `newLiteralExpression`、`newBinaryExpression`、`newNameExpression` 和 Proto/codegen 结构；这些正是
  前文已列出的函数体 AST / codegen arena 生命周期问题。

结论：当前 HEAD 没有新的低风险编译期小切口。继续降低 `compile_3000_functions` 必须先进入能减少
parser AST 构造或显著降低 codegen arena 空间的较大设计；继续围绕 token 分派、Source 读取、容量 hint
或 codegen-only 直发做局部调参，不符合本阶段验收。
...
### 6. `string_concat`：builder 后剩余 profile

`string_concat` 已实现窄形态 builder，并在 `f5f9028` 后三轮完整基线中稳定接近官方：
`1.03x / 1.03x / 1.05x`，平均约 `1.04x`。该项已进入噪声带，后续不应继续围绕字符串拼接扩张
fast path，除非新的完整三轮基线显示稳定回退且有 profile 指向明确主因。

2026-07-04 在 `525aafb` 后补跑当前 residual profile：

```bash
CGO_ENABLED=0 go test ./lua -run '^$' -bench '^BenchmarkDoStringStringConcat$' \
  -benchmem -benchtime=5s -count=1 \
  -cpuprofile /tmp/go-lua-vm-next-profiles/string_concat_after_525aafb_cpu.pprof \
  -memprofile /tmp/go-lua-vm-next-profiles/string_concat_after_525aafb_mem.pprof
go tool pprof -top /tmp/go-lua-vm-next-profiles/string_concat_after_525aafb_cpu.pprof
go tool pprof -top -alloc_objects /tmp/go-lua-vm-next-profiles/string_concat_after_525aafb_mem.pprof
go tool pprof -top -alloc_space /tmp/go-lua-vm-next-profiles/string_concat_after_525aafb_mem.pprof
```

- 单轮 profile：约 `80.83 us/op`、`311.3 KB/op`、`370 allocs/op`。
- 五轮 micro：约 `78.04 / 81.80 / 83.96 / 86.07 / 86.66 us/op`、`311.3 KB/op`、`370 allocs/op`。
- CPU profile 受 GC/runtime 调度噪声影响较大；可归因执行侧中
  `TryExecuteStringAppendForLoopBatch` / `repeatedAppendString` 已是主要命中路径，普通 `executeConcat`
  只剩约 `3.59%` cum。
- alloc profile 中 `strings.Builder.Grow` / `internal/bytealg.MakeNoZero` 是最大项，说明剩余主分配是
  builder 最终字符串空间；`DoString` 口径还包含 `OpenLibs`、parser/codegen 和标准库 table 初始化。

结论：相比 builder 前约 `435.49 us/op`、`2.25 MB/op`、`2196 allocs/op`，当前短命字符串分配已经被消除；
剩余成本主要是最终字符串不可变结果、benchmark 的 `DoString` 编译/开库成本，以及少量 guard 回退/普通
...
- [x] 在 `b2233be` 后重新 profile `compile_3000_functions`，确认当前 HEAD 仍无低风险编译期小切口。
