...
- `native_modules` 是显式可选构建能力，需要 `CGO_ENABLED=1` 和 `-tags native_modules`；该构建当前已在 macOS arm64 上用仓库内 `lua-cjson` 源码完成 `.so` 与 `.dylib` 运行期验收，覆盖 `require("cjson")`、`encode/decode`、`cjson.null` 和错误输入 `pcall`。
...
- `native_modules` 的官方 Lua 5.3 ABI 现成二进制模块验收尚未闭环；Linux `.so` 和 Windows `.dll` 运行期验收仍待目标平台或 CI，Windows 还需要 `lua53.dll` shim 或等价 import library 方案。
