...
- Native module 是 CGO 能力，使用 `CGO_ENABLED=1` 即自动启用，不需要自定义 build tag；该构建当前已在 macOS arm64、Linux arm64、Windows amd64 与 Android arm64 上用仓库内 fixture、lua-cjson、LPeg 和 LuaSocket 完成目标平台源码构建与运行期验收。macOS 覆盖 `.so` / `.dylib`，Linux 覆盖 `.so`，Windows 覆盖 `.dll` 与 `lua53.dll` runtime shim/import library，Android 覆盖设备侧 `.so` 真实模块主路径。
...
- Native CGO 能力只面向按 Lua 5.3 public C API 编写并导出 `luaopen_*` 的 Lua C 模块，不是任意动态库 FFI；依赖 `lstate.h`、`lobject.h` 等 Lua 内部头文件或访问 `lua_State` 内部布局的模块不属于兼容承诺。
- Native CGO 构建已在 macOS arm64、Linux arm64 与 Windows amd64 上通过仓库内真实模块源码构建与运行期验收；其他 OS/架构组合、Android 真实第三方模块全量验收，以及未经源码固定的现成第三方二进制模块仍需独立矩阵确认。
...
- CGO 只服务 Lua C 模块加载；纯 Go 构建与嵌入 API 仍以 `CGO_ENABLED=0` 路径验收。
...
- 评估是否提供公开 native loader 适配包：要求 `CGO_ENABLED=0` 跨平台构建不受影响，平台相关实现必须隔离在 Go 工具链 `cgo`/平台约束或宿主适配层中。
