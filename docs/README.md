...
GLua 不修改 Lua 5.3 的核心语言目标。默认模式保留 Lua 5.3.6 语义，同时提供可以在运行时关闭的扩展：
...
- `CGO_ENABLED=1` 构建自动提供的 Lua 5.3 C 模块加载。
...
- Native 构建只需使用 `CGO_ENABLED=1`，不需要自定义 build tag，并需要当前平台可用的 C 工具链，参见 [Native 三平台构建](NATIVE_BUILD_GUIDE.md)。
