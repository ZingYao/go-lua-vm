...
> 非商业用途可按 PolyForm Noncommercial 1.0.0 免费使用；商业用途需要向 `zing` 取得单独付费授权。完整条款见扩展内 `LICENSE` 和项目根目录 `COMMERCIAL_LICENSE.md`。
...
- 支持 glua 扩展语法格式化。
- 提供内置函数和自定义函数补全。
- 提供 `glua.event`、JSON/YAML/XML/TOML 序列化、codec、hash、regex、UUID、ZIP、path 和 schema API 的补全、定义跳转及中英文悬停文档。
