...
emit_probe_prebuild_parts() {
  cat <<'LUA'
local attempts = {}
local probe_open = '[' * m.Cg(m.P'='^0, "init") * '['
local probe_close = m.Cmt(']' * m.C(m.P'='^0) * ']' * m.Cb("init"), function (_, pos, s1, s2)
                                               attempts[#attempts + 1] = pos .. ':' .. s1 .. ':' .. s2
                                               return s1 == s2 end)
local probe_any = m.P(1)
LUA
}
...
LUA
  ...
  case "${PROBE_PREBUILD_DUMMY_VALUE:-}" in
    "")
      ;;
    string)
      cat <<'LUA'
dummy_value = "dummy-prebuild"
LUA
      ;;
    number)
      cat <<'LUA'
...
LUA
    ...
    table)
      cat <<'LUA'
dummy_value = { marker = "dummy-prebuild" }
LUA
      ;;
    boolean)
      cat <<'LUA'
...
LUA
      ;;
    *)
      echo "unknown PROBE_PREBUILD_DUMMY_VALUE=${PROBE_PREBUILD_DUMMY_VALUE}" >&2
      return 1
      ;;
  esac
}
...
LUA
  ...
  if [[ "${PROBE_SELECTED_WARMUP_MATCH:-0}" == "1" ]]; then
    cat <<'LUA'
local _, probe_warmup_match_result = pcall(function()
  return probe_warmup:match("]]")
end)
probe_warmup_match_result = nil
LUA
  fi
  cat <<'LUA'
probe_warmup = nil
