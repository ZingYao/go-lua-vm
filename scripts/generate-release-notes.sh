...

{
  ...
  echo "## English"
  echo
  echo "This release publishes unified full-feature CLI artifacts for non-macOS targets built with \`CGO_ENABLED=1\`; no custom Go build tags are required. Native Lua C module loading is included automatically in those builds. macOS native artifacts are built locally and uploaded separately."
  echo
  ...
  echo "### Highlights"
  echo
  echo "- Improved beyond stock Lua 5.3 as a pure-Go VM distribution: compatible Lua 5.3 behavior is paired with embeddable Go APIs, \`glua\`/\`gluac\`/\`gluals\` command-line tools, DAP debugging, editor integration, bytecode inspection, formatting, and automatic native Lua C module loading in CGO-enabled builds."
  ...
  echo "- Release CLI artifacts enable CGO for Linux, Windows, and Android without custom build tags; macOS CGO packages are produced from a local Mac build."
  ...
  echo "## 中文"
  echo
  echo "本次发布会为非 macOS 目标生成统一的全功能 CLI 产物，构建时只需启用 \`CGO_ENABLED=1\`，不再需要任何自定义 Go build tag；Lua C 原生模块加载能力会自动包含。macOS native 产物由本机编译后单独上传。"
  echo
  ...
  echo "### 更新重点"
  echo
  echo "- 在官方 Lua 5.3 行为兼容基础上提供纯 Go VM 发行能力：同时包含可嵌入 Go API、\`glua\`/\`gluac\`/\`gluals\` 命令行工具、DAP 调试、编辑器集成、字节码查看、格式化，以及 CGO 构建中自动启用的 Lua C 原生模块加载。"
  ...
  echo "- Linux、Windows 和 Android 的 CLI 发布产物均启用 CGO 且不使用自定义 build tag；macOS CGO 包由本机 Mac 构建补充。"
