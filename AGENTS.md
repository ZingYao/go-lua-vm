...
- 出现功能、兼容、性能或平台问题后，每一轮排查开始前必须先评估本轮查询方案，明确假设、验证方法、预期能排除或确认的范围，以及失败后的切换策略。
- 排查不能长期依赖单一手段；应在源码阅读、gopls 符号/引用分析、最小复现、二分法、差分法、矩阵对照、trace/log、profile、官方 Lua 行为对比、golden 对比和脚本化回归之间灵活切换。
- 连续两轮使用同类方法没有缩小范围时，必须暂停并重新评估方法，优先切换到二分、差分或最小复现；不得继续堆叠人工猜测和线性控制组。
- 能脚本化的排查必须尽快固化为可复跑脚本或定向测试；脚本输出必须包含输入范围、关键环境、判定结果和 good/bad 分类，便于后续复核。
- 每轮排查完成后必须记录本轮证实点、证伪点、剩余假设和下一轮推荐方法；若新证据推翻旧结论，必须在 TODO 或相关文档中明确修正旧结论，避免沿用过宽或过时判断。
