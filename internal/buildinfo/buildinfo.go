...
func FeatureText(includeDAP bool) string {
	...
	if native.Enabled() {
		// CGO 构建下 package.loadlib 会接入真实动态库 loader。
		builder.WriteString(localize.Text("enabled by CGO\n", "已通过 CGO 启用\n"))
	} else {
...
func nativeFeatureText() string {
	// native.Enabled 由 Go 工具链的 cgo 能力文件提供，避免 CLI 输出与真实构建能力漂移。
	if native.Enabled() {
